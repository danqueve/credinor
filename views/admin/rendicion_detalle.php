...
<div class="max-w-4xl mx-auto space-y-6 pb-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="<?= url('admin/rendiciones/' . $rendicion['id'] . '/pdf') ?>"
           class="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-700 text-white font-bold px-4 py-2.5 rounded-xl transition-all text-sm shadow-sm active:scale-95">
    ...
    <?php
        $efvo = 0.0; $transf = 0.0;
        foreach ($rendicion['pagos'] as $_p) {
            if ($_p['metodo_pago'] === 'transferencia') $transf += (float)$_p['monto'];
            else $efvo += (float)$_p['monto'];
        }
    ?>
    <div class="grid grid-cols-2 gap-3 mb-1">
        <div class="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 text-center">
            ...
            <span class="font-bold text-xl text-emerald-700" style="font-family:'Outfit',sans-serif;"><?= MoneyHelper::format($efvo) ?></span>
        </div>
        <div class="bg-violet-50 border border-violet-100 rounded-2xl p-4 text-center">
            ...
            <span class="font-bold text-xl text-violet-700" style="font-family:'Outfit',sans-serif;"><?= MoneyHelper::format($transf) ?></span>
        </div>
    </div>
